...
def run_experiment(netG, dataloader_test, nn_path, opt, optimize_red_first=False, n_bfgs_iter=50, lbfgs_lr=0.05, num_lbfgs_trials=3):
    """
    ...
    for i_batch, data in enumerate(iterator_data):
        ...
        for i_trial in range(num_lbfgs_trials):
            print('Class {0}: {1}, trial {2} of {3}'.format(1, 'neutrophils', i_trial + 1, num_lbfgs_trials))
            sys.stdout.flush()
            # get the noise leading to the good reconstruction
            if optimize_red_first:
                noise_init, noise = reconstruct_cells_red_first(data, netG_forward, opt, n_bfgs_iter=n_bfgs_iter, lbfgs_lr=lbfgs_lr)
            else:
                print ('Optimize red first else')
                noise_init, noise = reconstruct_cells(data, netG_forward, opt, n_bfgs_iter=n_bfgs_iter, lbfgs_lr=lbfgs_lr)
            ...
            for i_im in range(reconstructions.size(0)):
                ...
                l2_dist = compute_rec_error(im_to_01(data[i_im].data), im_to_01(reconstructions[i_im].data))
        ...
        for i_im in range(reconstructions_best.size(0)):
            ...
            _, nn_ids = nearest_neighbors.find_neighbors(ref_im_01, search_images['neutrophils'], search_image_names['neutrophils'], num_neighbors=1)
        ...
        # save results
        for i_im in range(reconstructions_best.size(0)):
            all_images = [data[i_im], reconstructions_best[i_im], reconstructions_best_init[i_im], neighbors[i_im]]
            all_images = torch.stack(all_images, 0)
            all_images = pad_channels(all_images.data, 1)
            file_name = os.path.join(opt.experiment, 'neutrophils', '{0}_batch{1}_image{2}.png'.format(class_name, i_batch, i_im))
